...
import { IMarketplaceChartData, IMarketplaceOrderHttpData } from '../../../Interfaces/marketplace.interface';
...
export class MostSoldItemComponent implements OnInit {
  ...
  posMostSoldItemsData: IPosMostSoldItemChartData[] = [];
  marketplaceMostSoldItemsData: IMarketplaceChartData[] = [];
  ...
  async executePosChart() {
    try {
      const data = await this.orderService.getAllPosOrders().toPromise() as IOrder[]
      if (data) {
  ...
  async executeMarketplaceChart() {
    const data = await this.orderService.getAllMarketplaceOrders().toPromise() as IMarketplaceOrderHttpData
